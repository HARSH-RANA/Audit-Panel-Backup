    ...
    jhAddRow: function (rowObj) {
        for (var c in rowObj) {
           ...
           // console.log(cellObj);
            ...
            var i = 0;
            for (var index in cellObj) {            
                //Now convert each value to string and comma-seprated
                if(c == 0) cellText = index;
                else cellText = cellObj[index];
                //console.log('ct:'+cellText);
                var cell = {v: cellText};
                if (cell.v === null)
                    continue;
                var cell_ref = XLSX.utils.encode_cell({c: i, r: this.rowCount});
